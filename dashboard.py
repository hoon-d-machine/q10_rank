...
@st.cache_data
def convert_df(df):
    # utf-8-sig로 인코딩해야 엑셀에서 한글이 안 깨집니다.
...
if df.empty:
    ...
    sel_event = st.sidebar.selectbox("행사(SID) 선택", events, index=0)
    
    # 행사 필터링
    df_event = df[df['event_sid'] == sel_event]
    ...
    csv = convert_df(final_df) # 현재 필터링된 데이터만 변환
    ...
    st.sidebar.download_button(
        label="🔍 필터된 데이터 받기",
        data=csv_filtered,
        file_name=f'Filtered_{sel_event}_{sel_cat}.csv',
        mime='text/csv',
    )

    # 2. 필터 없이 전체 원본 받기 (모든 행사, 모든 카테고리 포함)
    st.sidebar.write("---") # 구분선
    csv_full = convert_df(df) # df는 로드한 전체 원본 데이터입니다.
    st.sidebar.download_button(
        label="💾 전체 원본 데이터 받기 (필터 X)",
        data=csv_full,
